...
export default function RoleUser() {
    ...
    const fetchRoleUser = async () => {
        setLoading(true);
        try {
            const { data, status, error } = await apiRequest(`/api/user?limit=${15}&page=${page + 1}`, 'GET');
            if (status === 200) {
    ...
    const debouncedSearch = useMemo(() => debounce(async (value) => {
        setLoading(true);
        try {
            const { data, status, error } = await apiRequest(`/api/user?search=${value}`, 'GET');
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Box direction="row" spacing={1}>
                    <IconButton onClick={() => navigate(`/admin/role-user/edit-role/${params.row._id}`)} disabled={!canEdit}>
                        <CreateOutlinedIcon fontSize="medium" />
                    </IconButton>
                    <IconButton onClick={() => navigate(`/admin/role-user/permission/${params.row._id}`)} disabled={!canEdit}>
                        <LockOpenOutlinedIcon fontSize="medium" />
                    </IconButton>
                    <IconButton>
                        <RemoveRedEyeOutlinedIcon fontSize="medium" />
                    </IconButton>
                    <IconButton onClick={() => handleDeleteRole(params.row)} disabled={!canEdit}>
        ...
        <Box style={{ padding: "1.25rem", height: "100%" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    />
                    {canEdit && (
                        <Button sx={{ bgcolor: "black", color: "white", fontWeight: '500' }} variant="outlined" startIcon={<AddOutlinedIcon />} onClick={() => navigate("/admin/role-user/add-role")}>
                            Add User
                        </Button>
                    )}
                </div>
